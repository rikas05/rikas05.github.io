...
const Contact = () => {
  ...
  return (
    <section id="contact" className="py-24 bg-gray-900 relative overflow-hidden">
      <div className="absolute top-1/2 -translate-y-1/2 left-0 w-72 md:w-96 h-72 md:h-96 bg-cyan-500/10 rounded-full filter blur-3xl animate-pulse"></div>
      <div className="absolute top-1/2 -translate-y-1/2 right-0 w-72 md:w-96 h-72 md:h-96 bg-purple-500/10 rounded-full filter blur-3xl animate-pulse" style={{animationDelay: '2s'}}></div>
          ...
          <div className="lg:col-span-1 space-y-4 md:space-y-6">
            {contactInfo.map((item) => (
              <a 
                key={item.value} 
                href={item.href}
                className="flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-gray-800/40 backdrop-blur-sm rounded-2xl border border-gray-700/60 transition-all duration-300 hover:border-cyan-400/50 hover:bg-gray-800/60"
              >
            ...
            <div className="flex justify-center pt-2 md:pt-4">
                {socialLinks.map((social) => (
                    <a key={social.href} href={social.href} target="_blank" rel="noopener noreferrer" className="p-2 md:p-3 bg-gray-800/40 backdrop-blur-sm rounded-full border border-gray-700/60 text-gray-400 hover:text-white hover:border-cyan-400/50 hover:bg-gray-800/60 transition-all duration-300 mx-1 md:mx-2">
