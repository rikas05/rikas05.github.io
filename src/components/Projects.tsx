            ...
            >
              ...
              <div className="p-6 md:p-8">
                <div className="flex items-center gap-3 md:gap-4 mb-3 md:mb-4">
                  <div className={`p-2 md:p-3 rounded-lg bg-gradient-to-br ${project.gradient}`}>
                    <project.icon className="h-7 w-7 md:h-8 md:w-8 text-white" />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
                    <div>
                        <h4 className="text-base md:text-lg font-semibold text-cyan-300 mb-2 md:mb-3">Key Features</h4>
                        <ul className="space-y-1 md:space-y-2">
                            {project.features.map((feature) => (
                                <li key={feature} className="flex items-center text-gray-300 text-xs md:text-sm">
                                    <div className="w-1 h-1 md:w-1.5 md:h-1.5 bg-blue-400 rounded-full mr-2 md:mr-3 flex-shrink-0"></div>
                                    {feature}
                                </li>
                            ))}
                        </ul>
                    ...
                    <div>
                        <h4 className="text-base md:text-lg font-semibold text-cyan-300 mb-2 md:mb-3">Technologies</h4>
                        <div className="flex flex-wrap gap-1 md:gap-2">
                            {project.technologies.map((tech) => (
                            <span key={tech} className="px-2 md:px-3 py-0.5 md:py-1 bg-gray-700/50 text-gray-300 text-xs rounded-full">
                                {tech}
                            </span>
                            ))}
                        </div>
                ...
                <div className="flex gap-3 md:gap-4 mt-4 md:mt-6 border-t border-gray-700/60 pt-4 md:pt-6">
                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 md:gap-2 px-3 md:px-4 py-2 bg-gray-700/50 hover:bg-gray-700 text-white rounded-lg transition-colors duration-300 text-xs md:text-base">
                        <Github className="h-4 w-4 md:h-5 md:w-5" />
                        <span>Code</span>
                    </a>
                    {project.live && (
                        <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 md:gap-2 px-3 md:px-4 py-2 bg-cyan-500/80 hover:bg-cyan-500 text-white rounded-lg transition-colors duration-300 text-xs md:text-base">
